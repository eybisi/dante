...
// Import log so we can log errors
...
public class EasySettings
{
										  ...
										  ArrayList<SettingsObject> settingsList)
	{
		...
		for (SettingsObject settObj : settingsList)
		{
			...
			SettingsObject.SettingsListener listener = settObj.listener;
			if(listener != null)
			{
				// Get value from shared preferences
				Object value = null;
				switch (settObj.getType())
				{
					case BOOLEAN:
					...
					case FLOAT:
						...
						break;
					case INTEGER:
						value = prefs.getInt(key, (int) defaultValue);
						break;
					case LONG:
						value = prefs.getLong(key, (long) defaultValue);
						break;
					case STRING:
						value = prefs.getString(key, (String) defaultValue);
						break;
					case STRING_SET:
						value = prefs.getStringSet(key, (Set<String>) defaultValue);
						break;
				}
				listener.onSettingsChanged(settObj, value,defaultValue);
			}
		}
